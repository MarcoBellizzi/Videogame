...
Questo è un progetto da usare come baseline per sviluppare un videogioco con Unity. L'obiettivo è fare un MVP in cui sono presenti tutte le funzionalità principali di un videogioco (movimento di un personaggio, gestione della camera, nemici, boss, menù...). Per ognuno di essi sono trattati capitoli specifici in cui passo dopo passo viene spiegata l'implementazione.

### Configurazione dell'ambiente

La versione di Unity usata è Unity 6.0 (6000.0.62f1). Come Ide di sviluppo usare VS Code. Per funzionare tutto correttamente bisogna settare da Unity VS Code come ide
...
Se tutto è corretto dovresti avere attivo l'Intellisense sugli script C# generati da Unity.

### Modelli

Quando si scarica un modello di un personaggio da mixamo, va copiato nella cartella Animation, poi va impostato il rig a umanoid e va generata la sua nuova maschera. La stessa maschera va usata in tutte le animazioni. Bisogna poi attaccare al modello (nell ispector) un Animator che ne gestisce le animazioni.

### Animazioni

Per ogni animazione che il modello intende fare va scaricata a parte l'animazione e messa dentro la relativa cartella in Animations.

Le animazioni del personaggio vanno settata come Rig -> Umanoid e impostare la maschera generata dal modello.

Per le animazione che si spostano ma lo spostamento viene gestito via script, bisogna mettere la spunta inplace e mettere tutto in bake into pose, per non spostare il trasform ma solo l'animazione.

Applicare anche il loop time alle animazione non statiche.

### Personaggio

Per la creazione di un personaggio base sono stati utilizzati modelli e animazione importati da Mixamo.

Si parte con un Empty Game Object. Ad esso è stato aggiunto il componente Character controller (utile per la gestione del movimento), RigidBody e uno script in cui vengono gestiti tutti i suoi movimenti e azioni. Dentro questo oggetto ci vanno un altro game object "Player orientation" utile per capire verso dove sta guardando il giocatore, i gameObject da fare seguire alle varie videocamere. Viene inserito anche il modello da mixamo, che risulterà rigido e con l'animazione base con cui è stato scaricato.

Per gestire l'animazione viene creato un Animator (aggiunto come componente). Qui dentro va passato il riferimento di un'animazione. Al momento la struttura di un animator di un player è strutturata in layer per le animazione base. Quello base è composto solo da un Blend Tree, un oggetto che definisce l'animazione in base ad un parametro, in questo caso Speed, e definisce se il personaggio è in attesa, cammina o corre. Per passare da un'animazione all'altra bisogna passare dallo script. Nello script del Personaggio, da qualche parte dentro il metodo Update (che viene chiamato ad ogni frame) c'è un animator.setFloat("Speed"...) con cui si comunica il valore della velocità, in base a quel valore il Blend Tree anima il personaggio con una delle tre animazioni. Per il passaggio da un'animazione all'altra si usano i trigger.
...
In questo progetto sono presenti 3 camera, una FreeLook per gli spostamenti base, un VirtualCamera per tenere d'occhio un oggetto, e un altra Virtual Camera per la modalità FPS.
